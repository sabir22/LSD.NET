...
// Edited by Sabich Gregory (c) 2018 <sabir22@mail.ru>
// for using with .net as DllImport library
...
*/
double * lsd_scale(int * n_out, double * img, int imgSize, int X, int Y, double scale)
{
	//return lsd_scale_region(n_out,img,X,Y,scale,NULL,NULL,NULL);
	// LSD parameters
	double * image = (double *)malloc(imgSize * sizeof(double));
	for (int i = 0; i < imgSize; i++)
	{
		image[i] = img[i];
	}
	double * out;
	out = lsd_scale_region(n_out, img, X, Y, scale, NULL, NULL, NULL);
	free(image);
	return out;
}
...
double * lsd(int * n_out, double * img, int imgSize, int X, int Y)
{
	// LSD parameters
	...
	double * out;
	out = lsd_scale(n_out, image, imgSize, X, Y, scale);
	free(image);
	return out;
}
